...
void quat_to_euler(float* q) {
  // roll (x-axis rotation)
  double t0 = +2.0 * (q[0] * q[1] + q[2] * q[3]);
  double t1 = +1.0 - 2.0 * (q[1] * q[1] + q[2] * q[2]);
  euler[0] = atan2(t0, t1) * 180.0 / M_PI;

  // pitch (y-axis rotation)
  double t2 = +2.0 * (q[0] * q[2] - q[1] * q[3]);
  t2 = t2 > 1.0 ? 1.0 : t2;
  t2 = t2 < -1.0 ? -1.0 : t2;
  euler[1] = asin(t2) * 180.0 / M_PI;

  // yaw (z-axis rotation)
  double t3 = +2.0 * (q[0] * q[3] + q[1] * q[2]);
  double t4 = +1.0 - 2.0 * (q[2] * q[2] + q[3] * q[3]);
  euler[2] = atan2(t3, t4) * 180.0 / M_PI;
}
...
void setup() {
  ...
  SPI.begin();
  bool initialized = imu.initialize(SPI, 5);
  if (!initialized) {
...
void loop(){
  imu.try_read(imu_data);

  if (millis() - last_print_time > 500) {
    quat_to_euler(imu_data.orientation);
